...
export function installDevMock() {
  ...
  const messages: DeviceLinkMessage[] = [
    {
      id: 'demo-1', senderId: 'phone', senderName: 'Harris 的 iPhone', direction: 'incoming', kind: 'text',
    ...
    {
      id: 'demo-2', senderId: 'desktop', senderName: '工作电脑', direction: 'outgoing', kind: 'link',
    ...
    {
      id: 'demo-3', senderId: 'phone', senderName: 'Harris 的 iPhone', direction: 'incoming', kind: 'image', text: '刚拍的白板',
  ...
  const state: DeviceLinkState = {
    settings: {
      ...
      webdav: { enabled: false, baseUrl: '', username: '', hasPassword: false, hasSyncPassword: false, status: 'disabled' },
    },
    server: {
  ...
  const api: DeviceLinkApi = {
    ...
    async sendText(text: string) {
      const message: DeviceLinkMessage = { id: crypto.randomUUID(), senderId: 'desktop', senderName: state.settings.deviceName, direction: 'outgoing', kind: /^https?:\/\//.test(text) ? 'link' : 'text', text, attachments: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: 'sent' }
