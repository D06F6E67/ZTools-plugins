...
async function copyLink() {
  if (!props.server?.pairingUrl) return
  await navigator.clipboard.writeText(props.server.pairingUrl)
  copied.value = true
...
<template>
  <div class="overlay" @mousedown.self="emit('close')">
    <section class="dialog pairing-dialog" role="dialog" aria-modal="true" aria-labelledby="pair-title">
      ...
      <p>让手机与电脑处于同一局域网，扫码后输入下方匹配码。</p>

      <template v-if="server?.running">
        <div class="qr-frame"><img :src="server.qrDataUrl" alt="设备配对二维码"></div>
        <div class="pair-code-label">本次匹配码</div>
        <div class="pair-code"><span v-for="(digit, index) in digits" :key="index">{{ digit }}</span></div>
        <div class="pair-address"><span>{{ server.accessUrl }}</span><button type="button" @click="copyLink"><Copy :size="14" />{{ copied ? '已复制' : '复制连接' }}</button></div>
      ...
      <p class="privacy-note">二维码包含一次性高熵密钥，匹配码不会直接在网络中传输。成功配对后二维码立即失效。</p>
    </section>
  </div>
</template>
