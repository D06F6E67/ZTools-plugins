...
const trendData = computed(() => {
  ...
  return {
    ...
    datasets: [
      ...
      { label: "缓存命中", type: "line", data: cache, borderColor: p(0.7), backgroundColor: p(0.12), tension: .3, fill: false, order: 1 },
    ],
  };
});
...
const trendOpts = computed(() => {
  ...
  return {
    ...
    plugins: { legend: { labels: { color: m, boxWidth: 12, padding: 12 } }, tooltip: { callbacks: { label: ctx => ctx.dataset.label + ": " + fmt(ctx.parsed.y) } } },
    scales: { x: { ticks: { color: m } }, y: { ticks: { color: m, callback: v => fmt(v) }, grid: { color: grid }, stacked: true } },
  };
});
...
const modelBarData = computed(() => {
  ...
  return {
    ...
    datasets: [{ data: list.map(m => m.total), backgroundColor: themeVars.primaryRgba(0.8), borderRadius: 4, barPercentage: .6 }],
  };
});
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          ...
          label(ctx) {
            const v = ctx.raw.v || 0;
            return v >= 1e4 ? (v / 1e4).toFixed(1) + " 万 tokens" : v.toLocaleString() + " tokens";
          },
        },
      },
    },
    scales: {
      ...
      y: {
        ...
        ticks: {
          color: muted,
          autoSkip: false,
          callback(v) { return WEEK_LABELS[v] || ""; },
        },
        grid: { display: false },
        border: { display: false },
      },
      ...
      <div class="panels">
        <div class="panel panel--wide">
          ...
          <div class="heatmap-wrap">
            <EChart type="matrix" :data="heatmapChartData" :options="heatmapChartOpts" height="150px" />
            <div class="heatmap-legend">
              <span class="legend-label">少</span>
              <span v-for="(a, i) in HEATMAP_LEVELS" :key="i" class="legend-box" :style="{ background: a === 0 ? 'rgba(148,163,184,.12)' : themeVars.primaryRgba(a) }"></span>
...
.heatmap-legend { display: flex; align-items: center; justify-content: flex-end; gap: 3px; margin-top: 6px; }
.legend-label { font-size: 11px; color: var(--text-muted); margin: 0 2px; }
