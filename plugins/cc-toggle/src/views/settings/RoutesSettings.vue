  ...
  <div class="routes-page">
    ...
    <div class="routes-hero" :class="{ 'routes-hero--running': isRunning }">
      ...
      <div v-else class="hero-url">
        ...
        <n-input-number v-model:value="portInput" :min="1024" :max="65535" size="small"
          class="port-input" :show-button="false" />
      ...
      <div v-if="isRunning" class="hero-info">
        ...
        <span class="hero-info-item">
          ...
          <span class="hero-info-value hero-info-value--code" :class="{ 'hero-info-value--muted': !activeModel }">
            {{ activeModel || "-" }}
          </span>
        </span>
      </div>
      ...
      <!-- 停止时：提示 -->
      <n-text v-else depth="3" style="font-size: 11px;">新端口将在下次启动代理时生效。从主页 TabBar 开启代理。</n-text>
    </div>
    ...
    <div v-if="isRunning" class="metrics-grid">
      <div class="metric-card">
        ...
        <div class="metric-value">{{ rt.activeConn || 0 }}<span class="metric-suffix"> 个</span></div>
...
.metric-accent--green { background: #22c55e; }
.metric-accent--blue  { background: #3b82f6; }
.metric-accent--amber { background: #f59e0b; }
.metric-accent--purple { background: #a855f7; }
.metric-value {
  font-size: 24px;
  font-weight: 700;
  line-height: 1.2;
