...
function save() {
  ...
  if (t === "claude") {
    ...
    if (form.sonnetModel) env.ANTHROPIC_DEFAULT_SONNET_MODEL = applyOneM(form.sonnetModel, role1M.sonnetModel);
    if (form.opusModel) env.ANTHROPIC_DEFAULT_OPUS_MODEL = applyOneM(form.opusModel, role1M.opusModel);
    if (form.haikuModel) env.ANTHROPIC_DEFAULT_HAIKU_MODEL = applyOneM(form.haikuModel, role1M.haikuModel);
    if (form.fableModel) env.ANTHROPIC_DEFAULT_FABLE_MODEL = applyOneM(form.fableModel, role1M.fableModel);
    if (form.subagentModel) env.CLAUDE_CODE_SUBAGENT_MODEL = applyOneM(form.subagentModel, role1M.subagentModel);
...
function openOAuthUrl(url) {
  try { window.ztools?.shellOpenExternal?.(url); } catch (e) { window.open(url, "_blank"); }
}
...
async function handleTestConnection() {
  ...
  testing.value = true;
  try {
    const result = await window.ztoolsCctoggle.testConnection(form.baseUrl, form.apiKey, tab.value);
...
async function handleFetchModels() {
  ...
  try {
    const result = await window.ztoolsCctoggle.fetchAvailableModels(form.baseUrl, form.apiKey, tab.value);
    if (result.success) {
...
// Catalog 表格列定义（Codex）
const cellInput = (model, key, placeholder) => h(NInput, {
  value: model[key],
...
const catalogColumns = [
  { title: '菜单显示名', key: 'displayName', minWidth: 92, render: (row) => cellInput(row, 'displayName', 'Ark Code Latest') },
  { title: '实际模型 ID', key: 'model', minWidth: 92, render: (row) => cellInput(row, 'model', 'ark-code-latest') },
  { title: '上下文长度', key: 'contextWindow', width: 88, render: (row) => cellInput(row, 'contextWindow', '256000') },
...
const role1M = reactive({ model: false, sonnetModel: false, opusModel: false, fableModel: false, haikuModel: false, subagentModel: false });
const claudeRoleColumns = [
  { title: '模型角色', key: 'role', width: 84, render: (row) => h('span', { style: 'font-size:11px;font-weight:600;' }, row.label) },
  { title: '实际模型 ID', key: 'model', minWidth: 140, flexGrow: 1, render: (row) => h(NInput, {
  ...
  { title: '声明支持 1M', key: 'ctx', width: 92, align: 'center', render: (row) => h(NCheckbox, {
...
const claudeRoleRows = computed(() => [
  ...
  { role: 'sonnet', label: 'Sonnet', field: 'sonnetModel', placeholder: 'claude-sonnet-4-20250514' },
...
const openclawColumns = [
  {
    type: 'expand',
    renderExpand: (row) => {
      return h('div', { style: 'display:flex;flex-direction:column;gap:2px;padding:8px 12px;max-width:280px;' }, [
        h('span', { style: 'font-size:11px;font-weight:600;color:var(--text-muted);' }, '上下文长度'),
        h(NInput, {
  ...
  { title: '菜单显示名', key: 'name', minWidth: 100, render: (row) => cellInput(row, 'name', 'Kimi K2.7 Code') },
  { title: '实际模型 ID', key: 'id', minWidth: 100, render: (row) => cellInput(row, 'id', 'kimi-k2.7-code') },
  {
    title: '', key: 'actions', width: 76, align: 'right',
    render: (row, rowIndex) => h('div', { style: 'display:flex;gap:4px;justify-content:flex-end;' }, [
      h(NButton, {
        circle: true, quaternary: true, size: 'small', disabled: rowIndex <= 0,
      ...
      h(NButton, {
        ...
        title: '删除此模型', onClick: () => removeOpenclawRow(rowIndex),
      }, { default: () => h(NIcon, { size: 14 }, { default: () => h(TrashOutline) }) }),
    ]),
  },
];
function openclawRowKey(row) { return openclawRows.value.indexOf(row); }
</script>
        ...
        <n-card title="连接配置" size="small" :bordered="true" class="section-card">
          <n-flex vertical :size="10">
            ...
            <n-alert v-if="form.authMethod!=='api_key'" type="warning" size="small">
              <template #default>
                <n-text>ZTools 环境暂不支持后端 token 交换，请在浏览器完成 OAuth 后手动粘贴 Access Token 到下方 API Key。</n-text>
                <n-button
        ...
        <n-card title="模型配置" size="small" :bordered="true" class="section-card">
          <n-flex vertical :size="10">
            ...
            <template v-else-if="tab==='claude'">
              <div>
                <n-text style="font-size: 11px; font-weight: 600; display: block; margin-bottom: 6px;">模型目录 <n-text depth="3" style="font-weight: 400;">(角色模型，/model 菜单显示)</n-text></n-text>
                <n-data-table
                ...
                />
                <n-text depth="3" style="font-size: 11px; margin-top: 4px; display: block;">主模型写入 ANTHROPIC_MODEL，其余角色写入对应默认模型变量，勾选 1M 后追加 [1M] 后缀，切换后需重启 Claude Code 生效。</n-text>
              </div>
            </template>
