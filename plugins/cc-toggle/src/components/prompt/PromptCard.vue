...
const { ALL_AGENTS, AGENT_LABELS, OPENCLAW_PROMPT_FILES, togglePromptAgent, applyPromptToAgent } = usePrompts();
...
const openClawPackApplied = computed(() => {
  return isPackPrompt.value && openClawAppliedFiles.value.length === openClawFullFiles.length
      ...
      >
        ...
        <div class="openclaw-menu">
          ...
          <div class="openclaw-menu__footer">
            <n-button
              v-if="openClawAssociated"
              size="tiny"
              quaternary
              @click="unbindOpenClaw"
            >
              取消关联
            </n-button>
            <n-button
