...
function getByPath(obj: any, pathStr: string): any {
  ...
  const parts: string[] = [];
  String(pathStr || "").split(".").forEach(function (seg) {
    const m = seg.match(/^([^[]*)((\[\d+\])*)$/);
    if (!m) return;
    if (m[1]) parts.push(m[1]);
    if (m[2]) {
      const idxs = m[2].match(/\[(\d+)\]/g) || [];
      idxs.forEach(function (i) { parts.push(i.replace(/[\[\]]/g, "")); });
    }
  });
  let cur: any = obj;
...
function getCurrency(data: any): string | undefined {
  const candidates = ["currency", "data.currency", "data.userInfo.currency", "data.currency_type", "balance_currency", "data.balance_currency", "balance_infos[0].currency"];
  for (const c of candidates) {
    const v = getByPath(data, c);
    if (typeof v === "string" && v) {
      const up = v.toUpperCase();
      if (up.indexOf("USD") !== -1 || up.indexOf("DOLLAR") !== -1) return "USD";
      if (up.indexOf("CNY") !== -1 || up.indexOf("RMB") !== -1 || up.indexOf("YUAN") !== -1) return "CNY";
    }
  }
  return undefined;
}
...
export class BalanceManager {
    ...
    return new Promise(function (resolve) {
      ...
      fetch(url, {
        ...
        .then(function (data) {
          ...
          BalanceManager.writeCache({ providerId: providerId, appType: appType, result: result, queriedAt: result.queriedAt });
          resolve(result);
        })
        .catch(function (e) {
  ...
  static async queryAllBalances(appType?: string): Promise<Record<string, BalanceResult>> {
    ...
    for (const app of apps) {
      let list: any[] = [];
      try { list = ProviderStore.listProviders(app); } catch (e) { list = []; }
      for (const p of list) {
        ...
        try {
          ...
          results[p.id] = { success: false, queriedAt: Date.now(), error: String(e && e.message || e) };
        }
      }
    }
    return results;
  ...
  private static parseBalance(data: any, cfg: ProviderBalanceConfig): { ok: boolean; balance?: number; used?: number; currency?: string; error?: string } {
