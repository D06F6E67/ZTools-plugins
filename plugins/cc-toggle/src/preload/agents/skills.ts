...
export class SkillManager {
  ...
  static toggleSkillToAgent(skillName: string, sourceApp: string, targetApp: string): OperationResult {
    var reg = SkillManager.getDeployRegistry();
  ...
  static removeProjectTarget(id: string): OperationResult {
    var targets = SkillManager.listProjectTargets().filter(function(t) { return t.id !== id; });
    _projectTargets = targets;
    ztools.dbStorage.setItem("ccswitch_project_targets", targets);
    return { success: true };
  }
  ...
  static removeSkillRepo(repoUrl: string): OperationResult {
    ztools.dbStorage.setItem('ccswitch_skill_repos', SkillManager.getSkillRepos().filter(function(r) { return r.url !== repoUrl; }));
    return { success: true };
  }
  ...
  static searchSkills(query?: string): Promise<SearchResult[]> {
    ...
    return new Promise(function(resolve) {
      try {
        var req = https.get(url, { timeout: 8000 }, function(res: any) {
          var data = "";
          res.on("data", function(c: string) { data += c; });
          res.on("end", function() {
            try {
              var json = JSON.parse(data);
              resolve((json.skills || []).map(SkillManager._mapSkill));
            } catch(e) {
              resolve([]);
            }
          });
  ...
  static installSkill(name: string, repo?: string, subPath?: string, branch?: string): OperationResult {
    try {
      if (!SkillManager._safeSkillName(name)) return { success: false, error: "invalid skill name" };
      var nest = SkillManager.getNestDir();
      ...
      SkillManager.setNestSkillMeta(name, { repo: repo || "", subPath: subPath || "", branch: branch || "main", installedAt: new Date().toISOString() });
  ...
  static removeNestSkill(skillName: string): OperationResult {
    if (!SkillManager._safeSkillName(skillName)) return { success: false, error: "invalid skill name" };
    var nest = SkillManager.getNestDir();
